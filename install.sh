...
# =============================================================================
# NAS 终极自动化影音管理系�?- 一键安装脚�?# 版本: v1.0.0
...
# 已安装组件追踪（用于回滚�?INSTALLED_COMPONENTS=()
...
error_exit() {
    log "ERROR" "$1"
    log "ERROR" "安装失败，请检查日志文�? $LOG_FILE"
    
    # 执行回滚
    if [ "${#INSTALLED_COMPONENTS[@]}" -gt 0 ]; then
        log "INFO" "检测到部分组件已安装，开始自动回�?.."
...
# 进度条显�?show_progress() {
    ...
    # 保存进度到文�?    echo "$percent|$step_name" > "$PROGRESS_FILE"
...
# 记录已安装组�?record_installed_component() {
    local component="$1"
...
validate_config() {
    ...
    for var in "${required_vars[@]}"; do
        if ! grep -q "^${var}=" "$config_file" || grep -q "^${var}=$" "$config_file" || grep -q "^${var}=your_.*_here" "$config_file"; then
            log "ERROR" "配置�?$var 未设置或使用默认�?
            validation_failed=true
        fi
    done
    
    # 检查端口冲�?    local ports=(3000 8001 8096 8080 9091 8088 19035 9780 25600 25378 25533 8083)
    for port in "${ports[@]}"; do
        if netstat -tuln 2>/dev/null | grep -q ":$port "; then
            log "WARNING" "端口 $port 已被占用，可能导致服务冲�?
        fi
    done
    
    # 检查目录权�?    if [ -d "$BASE_DIR" ] && [ ! -w "$BASE_DIR" ]; then
        log "ERROR" "数据目录 $BASE_DIR 不可�?
        validation_failed=true
    fi
...
pull_docker_images_parallel() {
    ...
    # �?docker-compose 文件中提取镜像名�?    if [ -f "docker-compose.core.yml" ]; then
    ...
    log "INFO" "找到 $total_images 个镜像需要拉�?
    ...
    # 等待所有任务完�?    while [ $current_parallel -gt 0 ]; do
        sleep 1
        for pid_file in "$pids_dir"/*.pid; do
            if [ -f "$pid_file" ]; then
                local pid=$(cat "$pid_file" 2>/dev/null)
    ...
    # 检查是否有失败的任�?    local failed_count=0
    ...
    if [ $failed_count -eq 0 ]; then
        ...
        log "WARNING" "$failed_count 个镜像拉取失败，将使用传统方式重�?
        # 退回到传统方式
        docker-compose -f docker-compose.core.yml --env-file "$ENV_FILE" pull
        docker-compose -f docker-compose.extend.yml --env-file "$ENV_FILE" pull
        record_installed_component "docker_images"
...
show_banner() {
    ...
    cat << 'EOF'
╔══════════════════════════════════════════════════════════════════════════════╗
�?                                                                             �?�?           NAS 终极自动化影音管理系�?- 一键安装脚�?                        �?�?                                                                             �?�? ┌─────────────────────────────────────────────────────────────────────────�?�?�? �? 🎯 核心特�?                                                           �?�?�? �? �?全自动化流水�? 搜索 �?下载 �?整理 �?通知                           �?�?�? �? �?一站式数字生活中心: 影视、音乐、漫画、电子书                         �?�?�? �? �?PT 生态深度集�? 自动辅种、刷流、Cookie 同步                        �?�?�? �? �?极致用户体验: 统一导航�?+ 微信通知 + 多设备访�?                    �?�?�? └─────────────────────────────────────────────────────────────────────────�?�?�?                                                                             �?�? 版本: v1.0.0                                                               �?�? 支持: Ubuntu 20.04+, Debian 11+, CentOS 8+                                �?�?                                                                             �?╚══════════════════════════════════════════════════════════════════════════════╝
EOF
    echo -e "${NC}"
}

# 检测系统信�?detect_system() {
    update_progress "检测系统信�?
...
# 检查前置条�?check_prerequisites() {
    update_progress "检查前置条�?
    
    # 检查是否为root用户
    if [[ $EUID -eq 0 ]]; then
        log "WARNING" "检测到 root 用户，建议使用普通用户执�?
    ...
    # 检查内�?    if (( $(echo "$MEMORY < 4.0" | bc -l) )); then
        log "WARNING" "内存不足 4GB，部分服务可能运行缓�?
    fi
    
    # 检查磁盘空�?    local disk_gb=$(echo "$DISK_SPACE" | sed 's/G//')
    if (( $(echo "$disk_gb < 50" | bc -l) )); then
        error_exit "磁盘空间不足 50GB，无法继续安�?
    fi
    
    # 检查网络连�?    if ! ping -c 1 google.com &> /dev/null; then
...
install_docker() {
    ...
    # 添加用户�?docker �?    sudo usermod -aG docker $USER
    
    # 启动 Docker 服务
    sudo systemctl start docker
    sudo systemctl enable docker
    
    # 配置 Docker 镜像加�?    sudo mkdir -p /etc/docker
...
install_docker_compose() {
    ...
    # 下载并安�?    sudo curl -L "https://github.com/docker/compose/releases/download/${latest_version}/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
    sudo chmod +x /usr/local/bin/docker-compose
    
    # 创建软链�?    sudo ln -sf /usr/local/bin/docker-compose /usr/bin/docker-compose
...
setup_directories() {
    ...
    if [ -x "${SCRIPT_DIR}/setup-directories.sh" ]; then
        ...
        error_exit "找不到目录设置脚�? ${SCRIPT_DIR}/setup-directories.sh"
    fi
...
start_core_services() {
    ...
    # 检查服务状�?    check_service_status "core"
...
start_extend_services() {
    ...
    # 检查服务状�?    check_service_status "extend"
    
    log "SUCCESS" "扩展服务启动完成"
}

# 检查服务状�?check_service_status() {
    local service_type="$1"
    log "INFO" "检�?$service_type 服务状�?.."
...
show_install_result() {
    ...
    echo -e "${GREEN}�? �?Homepage 导航�?    http://$(hostname -I | awk '{print $1}'):3000                   �?{NC}"
    ...
    echo -e "${GREEN}�?                                                                             �?{NC}"
    echo -e "${GREEN}�? 📚 媒体库服�?                                                             �?{NC}"
    echo -e "${GREEN}�? �?Komga 漫画�?       http://$(hostname -I | awk '{print $1}'):25600                  �?{NC}"
    ...
    echo -e "${GREEN}�? �?Calibre 电子�?     http://$(hostname -I | awk '{print $1}'):8083                   �?{NC}"
    echo -e "${GREEN}�?                                                                             �?{NC}"
...
# 主安装流�?main() {
    ...
    # 用户确认
    echo -e "${YELLOW}即将开始安�?NAS 自动化系统，预计耗时 10-30 分钟${NC}"
    ...
    # 记录开始时�?    local start_time=$(date +%s)
    log "INFO" "开始安�?NAS 自动化系�?.."
    ...
    log "SUCCESS" "安装完成！总耗时: ${minutes}�?{seconds}�?
    log "INFO" "重启后服务将自动启动"
